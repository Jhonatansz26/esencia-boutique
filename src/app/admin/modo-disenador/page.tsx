...
import { getPageLayout, toDraftSlug } from "@/lib/page-layout";
...
const SECTION_REGISTRY: Record<string, React.ComponentType<Record<string, unknown>>> = {
  materials: Materials,
  "product-grid": ProductGridClient,
  values: Values,
  reviews: Reviews,
...
/**
 * Set of section types that support inline editable props (editable + onUpdateConfig).
 * Add a section type here once its component handles the `editable` / `onUpdateConfig` props.
 */
...
export default function ModoDisenadorPage() {
  ...
  const handleCanvasDragEnd = (event: DragEndEvent) => {
    ...
    updateSections((prev) => {
      const allOrdered = [...prev].sort((a, b) => a.order - b.order);
      const oldIndex = allOrdered.findIndex((s) => s.id === active.id);
      const newIndex = allOrdered.findIndex((s) => s.id === over.id);

      if (oldIndex === -1 || newIndex === -1) return prev;

      const reordered = arrayMove(allOrdered, oldIndex, newIndex);
  ...
  const handleSave = useCallback(async (updatedSections: SectionConfig[]) => {
    try {
      const success = await updatePageLayoutServer(updatedSections, toDraftSlug(activeSlug));
      if (success) {
        setOriginalSections(updatedSections);
        setMutationCount(0);
      } else {
        showToast("Error al guardar el borrador. Revisa las políticas RLS o tu conexión.", "error");
      }
      return success;
    } catch (err) {
  ...
  }, [hasUnsavedChanges, handleSave, sections, selectedSectionId, heroSection, orderedSections, handleUndo, handleRedo]);
              ...
              <div className="grid grid-cols-12 gap-4 px-4 py-6">
                ...
                >
                  <SectionCanvasWrapper
                    ...
                    isFirst={true}
                    isLast={orderedSections.length === 0}
                ...
                <SortableContext items={orderedSections.map(s => s.id)} strategy={rectSortingStrategy}>
                  <AnimatePresence mode="popLayout">
                    {orderedSections.map((section, index) => {
                      ...
                      const isEditable = EDITABLE_SECTION_TYPES.has(section.type);
                    ...
                    >
                        ...
                        {index < orderedSections.length - 1 && section.layoutColumns !== 6 && <SectionDivider />}
                      </motion.div>
                    );
                  })}
                  </AnimatePresence>
